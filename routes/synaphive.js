module.exports = function createSynapHiveRoutes({ requireAuth }) {
  ...
  const DATA_DIR = path.join(__dirname, '..', 'data', 'synaphive');
  ...
  const DEFAULT_COMMUNITY = [
    ...
    { platform: 'github',   label: 'GitHub',       followers: 0, icon: '⭐', url: 'https://github.com/Ashwin012/synaphive', updatedAt: new Date().toISOString() },
  ...
  router.get('/api/synaphive/stats', requireAuth, (req, res) => {
  ...
  router.get('/api/synaphive/activities', requireAuth, (req, res) => {
    res.json(readActivities());
  });

  router.post('/api/synaphive/activities', requireAuth, (req, res) => {
    const activities = readActivities();
    const activity = {
  ...
  router.put('/api/synaphive/activities/:id', requireAuth, (req, res) => {
    const activities = readActivities();
  ...
  router.delete('/api/synaphive/activities/:id', requireAuth, (req, res) => {
    const activities = readActivities();
    const idx = activities.findIndex(a => a.id === req.params.id);
    if (idx === -1) return res.status(404).json({ error: 'Not found' });
  ...
  router.get('/api/synaphive/community', requireAuth, (req, res) => {
    res.json(readCommunity());
  });

  router.put('/api/synaphive/community/:platform', requireAuth, (req, res) => {
    const community = readCommunity();
