...
def main():
    ...
    if all_new:
        if True:  # Always generate summaries via claude CLI
            print(f"\nGenerating AI summaries for {len(all_new)} articles...")
            for i, article in enumerate(all_new):
                print(f"  [{i+1}/{len(all_new)}] {article['title'][:60]}...")
                article['summary'] = generate_summary(article['title'], article['url'])
